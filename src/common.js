...
const formatRunMessage = (runResult) => (message) => {
    const formatted = `${runResult.data.name ? `${runResult.data.name}\n` : ""}${runResult.data.taskName
            ? `${runResult.data.taskName} - ${runResult.data.actorName}`
            : runResult.data.actorName
...
const createRunLink = ({ actorId, taskId, runId }) => {
    return `https://my.apify.com/${taskId ? "tasks" : "actors"}/${taskId || actorId
        }/runs/${runId}`;
};
...
const collectFailed = (result) => {
    ...
    const failed = Object.values(result).flatMap((v) => {
        ...
        total +=
            v?.specs?.reduce(
                (
                    out,
                    spec,
                ) => (out + spec.failedExpectations.length +
        ...
        if (
            !v?.specs?.length ||
            !v.specs.some((spec) => spec.failedExpectations.length)
        ) {
            passingSpecs++;
            return [];
        }
        ...
        return v.specs.flatMap((spec) => {
            ...
            return spec.failedExpectations.map((s) => {
                return {
                    ...
                    markdown: `\`\`\`${linkToFormat(s.message, (link) =>
                        `<${link}|${link.split("/").pop()}>`)
                        }\`\`\``,
                    html: linkToFormat(
                        s.message,
...
const createNotifier = (input) => {
    return async ({ emailMessage, slackMessage, subject }) => {
        ...
        if (input.email?.trim().includes("@") && emailMessage && subject) {
            ...
            try {
                await Apify.call("trudax/send-email", {
                    to: input.email.trim(),
                    subject,
                    text: "",
                    html: emailMessage,
